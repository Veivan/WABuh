package register;

import java.io.UnsupportedEncodingException;
import java.util.Base64;

import base.ApiBase;

public class WaToken {

	public static String GenerateRequestToken(String number, String platform) throws UnsupportedEncodingException {
		String result = "";
		switch (platform) {
		case "Nokia":
			String token = "PdA2DJyKoUrwLw1Bg6EIhzh502dF9noR9uFCllGk1447796090073"
					+ number;
			result = ApiBase.md5Custom(token).toLowerCase().replace("-", "");
			break;

		case "Android":
		    String st = "/UIGKU1FVQa+ATM2A0za7G2KI9S/CwPYjgAbc67v7ep42eO/WeTLx1lb1cHwxpsEgF4+PmYpLd2YpGUdX/A2JQitsHzDwgcdBpUf7psX1BU=";
			String signature = "";
			String classesMd5 = "U+w9L64wtuyknyhiDVdM/w==";

			byte[] key0 = Base64.getDecoder().decode(signature.getBytes("UTF-8"));
			byte[] key1 = Base64.getDecoder().decode(classesMd5.getBytes("UTF-8"));
			byte[] key2 = Base64.getDecoder().decode(st.getBytes("UTF-8"));

			String data = new String(key0, "UTF-8") + new String(key1, "UTF-8") + number;
			// str_repeat(char, 64)		
			String opad = new String(new char[64]).replace("\0", ""+(char)(0x5C));
			String ipad = new String(new char[64]).replace("\0", ""+(char)(0x36));
			
			/*
			 * $key2 = base64_decode(
			 * "/UIGKU1FVQa+ATM2A0za7G2KI9S/CwPYjgAbc67v7ep42eO/WeTLx1lb1cHwxpsEgF4+PmYpLd2YpGUdX/A2JQitsHzDwgcdBpUf7psX1BU="
			 * ); 
			 * $data = base64_decode($signature) . base64_decode($classesMd5)
			 * . $phone;
			 * 
			 * $opad = str_repeat(chr(0x5C), 64); 
			 * $ipad = str_repeat(chr(0x36), 64); 
			 * for ($i = 0; $i < 64; $i++) { $opad[$i] = $opad[$i] ^
			 * $key2[$i]; $ipad[$i] = $ipad[$i] ^ $key2[$i]; }
			 * 
			 * $output = hash("sha1", $opad . hash("sha1", $ipad . $data, true),
			 * true);
			 * 
			 * return base64_encode($output);
			 */
			result = "qq";
		}
		
		return result;
	}

}
